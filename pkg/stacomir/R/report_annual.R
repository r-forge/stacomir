...
setMethod("xtable",signature=signature("report_annual"),definition=function(x,
			...
			fn_title<-function(les_valeurs,valeur_uk,name_uk,total=TRUE){
				...
				if (length(les_valeurs)==1) {
					...
					repetes<-c(les_valeurs[2:length(les_valeurs)]==les_valeurs[1:(length(les_valeurs)-1)],FALSE) # FALSE, at the end we want the values aggregated anyway
				}
				rr=1
			...
			xt<-xtable::xtable(dat,caption=caption,
					label=label,
					align=align,
					digits=0,
