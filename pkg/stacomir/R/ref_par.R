...
# (nrow(object@data) > 0){   hpar=function(h,...){   
...
#   car_libelle=fun_char_spe(object@data$par_nom)
#   car_libelle[nchar(car_libelle)>30]<-paste(substr(car_libelle[nchar(car_libelle)>30],1,30),'.',sep='')
#   grouppar<-ggroup()   assign('gouppar',grouppar,envir=envir_stacomi)
#   add(notebook,grouppar,label=label)
#   frameparsource<-gframe(gettext('Select
# here',domain='R-stacomiR'),container=grouppar)   tbsourcepar =
# gtable(car_libelle,container=frameparsource,expand = TRUE, fill = TRUE)
#   size(tbsourcepar)<-c(160,300)   framepardest<-gframe(gettext('drop
# here',domain='R-stacomiR'),container=grouppar)   # need for a fixed size
# data.frame otherwise errors when adding new lines
#   xx<-data.frame(choice=rep('',8))   xx$choice<-as.character(xx$choice)
#   tbdestpar=gtable(xx,container=framepardest,expand = TRUE, fill = TRUE)
#   size(tbdestpar)<-c(160,300)   adddropsource(tbsourcepar)
#   adddroptarget(tbdestpar)    
#   adddropmotion(tbdestpar,handler=function(h,...) {   
# valeurs<-tbdestpar[,]    valeurs<-valeurs[valeurs!='']    if
# (!svalue(tbsourcepar)%in%valeurs){
...
#   adddropsource(tbdestpar)   adddroptarget(tbsourcepar)
#   removepar<-function(){    valeurs<-tbdestpar[,]   
# valeurs<-valeurs[valeurs!='']   
# valeurs<-valeurs[-match(svalue(tbdestpar),valeurs)]   
# tbdestpar[,]<-c(valeurs,rep('',8-length(valeurs)))   }
#   adddropmotion(tbsourcepar,handler=function(h,...) {    removepar()
#   })   addHandlerDoubleclick(tbdestpar,handler=function(h,...) {   
# removepar()   })   gbutton('OK', container = grouppar, handler = hpar)
#  } else {   funout(gettext('Error : no parameters in the database (the
# query returns 0 entry)\n',domain='R-stacomiR'),arret=TRUE)  } })
