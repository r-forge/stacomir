...
setMethod("connect", signature = signature("report_ge_weight"), definition = function(object) {
    ...
    requete@datedebut = strptime(paste(object@anneedebut@annee_selectionnee, "-08-01",
        sep = ""), format = "%Y-%m-%d")
    requete@datefin = strptime(paste(object@anneefin@annee_selectionnee, "-08-01",
        sep = ""), format = "%Y-%m-%d")
    ...
    requete@select = paste("SELECT lot_identifiant,ope_date_debut,ope_date_fin,lot_effectif,car_valeur_quantitatif as poids,",
        ...
        " date_part('month',ope_date_debut) as mois", " FROM ", rlang::env_get(envir_stacomi,
            "sch"), "vue_lot_ope_car_qan", sep = "")
    requete@and = paste(" AND ope_dic_identifiant=", object@dc@dc_selectionne, " AND std_libelle='civelle'",
...
##' @keywords internal
# hcalc = function(h,...) { r_gew<-get('report_ge_weight',envir=envir_stacomi)
# r_gew<-charge(r_gew) r_gew<-connect(r_gew) r_gew<-calcule(r_gew) }
...
##' @param \dots additional arguments passed to the function
##' @keywords internal
# hplot=function(h,...){ r_gew<-get('report_ge_weight',envir=envir_stacomi) if
# (h$action==1) { plot(r_gew,plot.type=1) } else if (h$action==2){
# plot(r_gew,plot.type=2) } else if (h$action==3){ plot(r_gew,plot.type=3)
# } }
    ...
    if (plot.type == 1) {
        ...
        plot(x = don$date, y = don$w, xlab = gettext("date", domain = "R-stacomiR"),
            ylab = gettext("mean weights", domain = "R-stacomiR"), col = "red", main = gettextf("Seasonal trend of %s, from %s to %s",
                type_poids, r_gew@anneedebut@annee_selectionnee, r_gew@anneefin@annee_selectionnee,
...
setMethod("model", signature(object = "report_ge_weight"), definition = function(object,
    ...
    if (model.type != "manual") {
        ...
        if (get("database_expected", envir_stacomi)) {
            fileout = paste(get("datawd", envir = envir_stacomi), "import_coe", r_gew@anneedebut@annee_selectionnee,
                r_gew@anneefin@annee_selectionnee, ".csv", sep = "")
