...
#' @keywords classes
#' @export
setClass(Class = "report_species", representation = representation(dc = "ref_dc",
    anneedebut = "ref_year", anneefin = "ref_year", data = "data.frame", calcdata = "data.frame",
    split = "ref_list"), prototype = prototype(dc = new("ref_dc"), anneedebut = new("ref_year"),
    anneefin = new("ref_year"), data = data.frame(), calcdata = data.frame(), split = new("ref_list")))
...
setMethod("choice_c", signature = signature("report_species"), definition = function(object,
    dc, split = "none", anneedebut, anneefin, silent = FALSE) {
    ...
    bilesp@anneedebut <- charge(object = bilesp@anneedebut, objectreport = "report_species")
    bilesp@anneedebut <- choice_c(object = bilesp@anneedebut, nomassign = "start_year",
        annee = anneedebut, silent = silent)
    bilesp@anneefin@data <- bilesp@anneedebut@data
    bilesp@anneefin <- choice_c(object = bilesp@anneefin, nomassign = "end_year",
        annee = anneefin, silent = silent)
    assign("bilesp", bilesp, envir = envir_stacomi)
    return(bilesp)
})
